	...
	struct TSP_NODE_BUF *node)
{
	...
	if (flag & (NODE_FLAG_RAW | NODE_FLAG_BASE)) {
		...
		for (i = 0; i < node->len; i++) {
			ret = ist30xx_read_buf(data->client, addr, tmp_rawbuf, 1);
			if (unlikely(ret))
				goto reg_access_end;

			addr += IST30XX_DATA_LEN;
			tmp_rawbuf++;
		}
	}
	...
	if (flag & NODE_FLAG_FILTER) {
		tsp_info("Reg addr: %x, size: %d\n", addr, node->len);
		for (i = 0; i < node->len; i++) {
			ret = ist30xx_read_buf(data->client, addr, tmp_fltbuf, 1);
			if (unlikely(ret))
				goto reg_access_end;
...
ssize_t ist30xx_read_ic_inform(struct device *dev, struct device_attribute *attr,
			char *buf)
{
	struct ist30xx_data *data = dev_get_drvdata(dev);
				...
				const char *buf, size_t size)
{
	...
	if (direct->cmd == DIRECT_CMD_WRITE) {
		...
		ist30xx_write_cmd(data->client, DIRECT_ADDR(direct->addr),
			direct->val);
			    ...
			    char *buf)
{
	...
	while (len > 0) {
		...
		for (i = 0; i < max_len; i++) {
			ret = ist30xx_read_buf(data->client, addr, &buf32[i], 1);
			if (unlikely(ret)) {
...
static DEVICE_ATTR(printk5, S_IRUGO, ist30xx_printk5_show, NULL);
static DEVICE_ATTR(printk6, S_IRUGO, ist30xx_printk6_show, NULL);
#if IST30XX_GESTURE
...
static DEVICE_ATTR(clb, S_IRUGO, ist30xx_calib_show, NULL);
static DEVICE_ATTR(clb_result, S_IRUGO, ist30xx_calib_result_show, NULL);
static DEVICE_ATTR(tsp_power_on, S_IRUGO, ist30xx_power_on_show, NULL);
...
/* sysfs : tunes */
static DEVICE_ATTR(node_info, S_IRUGO, tunes_node_info_show, NULL);
...
int ist30xx_init_misc_sysfs(struct ist30xx_data *data)
{
	...
	ist30xx_frame_buf = kmalloc(frame_buf_size, GFP_KERNEL);
